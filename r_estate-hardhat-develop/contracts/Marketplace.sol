...
contract Marketplace is ReentrancyGuard {
    ...
    // }
function getAllListings() external view returns (Listing[] memory) {
    uint256 total = _totalListings.current();
    uint256 unsoldCount = total - _listingSold.current();

    Listing[] memory items = new Listing[](unsoldCount);
    uint256 index = 0;

    for (uint256 i = 1; i <= total; i++) {
        if (listings[i].owner == address(0)) {
